import { Anchor, Image, Text } from "@mantine/core";
import styles from "./floating-component.module.scss"


const FloatingComponent = () => {
    return (
        <>
            <Anchor className={styles.floatingWhatsapp} href={'https://wa.me/+917907856810'} target="_blank">
                <svg
                    id="Layer_1"
                    data-name="Layer 1"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 85.14 85.14"
                    className={styles.imageWidth}
                >
                    <path d="M42.57,0C19.06,0,0,19.06,0,42.57s19.06,42.57,42.57,42.57,42.57-19.06,42.57-42.57S66.08,0,42.57,0ZM42.55,66.55h-.01c-4.26,0-8.46-1.07-12.2-3.11l-13.53,3.55,3.62-13.23c-2.23-3.87-3.41-8.26-3.41-12.76,0-14.07,11.46-25.52,25.53-25.52,6.78-.02,13.28,2.68,18.06,7.49,4.8,4.78,7.49,11.28,7.47,18.06,0,14.08-11.46,25.53-25.53,25.53Z" />
                    <path d="M42.56,19.79c-11.7,0-21.22,9.52-21.22,21.21,0,3.99,1.12,7.91,3.24,11.29l.5.8-2.14,7.83,8.03-2.11.77.46c3.27,1.94,7,2.96,10.8,2.96h0c11.69,0,21.21-9.52,21.22-21.21.02-5.63-2.22-11.04-6.21-15.01-4-4.01-9.33-6.22-15-6.22ZM55.04,50.12c-.53,1.49-3.08,2.85-4.31,3.03-1.1.17-2.49.23-4.02-.25-.93-.29-2.11-.68-3.64-1.34-6.4-2.76-10.58-9.21-10.9-9.63-.32-.43-2.6-3.46-2.6-6.6s1.65-4.68,2.23-5.32c.58-.64,1.27-.8,1.7-.8s.85,0,1.22.02c.39.02.92-.15,1.44,1.09.53,1.28,1.8,4.42,1.97,4.74.16.32.27.69.05,1.12-.21.43-.32.69-.64,1.07-.32.37-.67.83-.96,1.12-.32.32-.65.66-.28,1.3.37.64,1.65,2.73,3.55,4.42,2.44,2.17,4.49,2.84,5.13,3.17.64.32,1.01.27,1.38-.16.37-.42,1.59-1.86,2.02-2.5.43-.64.85-.53,1.44-.32.58.21,3.72,1.76,4.36,2.08h0c.64.32,1.06.48,1.22.74.16.27.16,1.54-.37,3.03Z" />
                </svg>
            </Anchor>
        </>
    )
}

export default FloatingComponent;